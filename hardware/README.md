...
Both scripts run with the Python *inside* KiCad, not the system one, because
the layout is built through KiCad's own `pcbnew` module. Where that lives:

| | |
| :-- | :-- |
| Windows | `%LOCALAPPDATA%\Programs\KiCad\<version>\bin\python.exe` |
| macOS | `/Applications/KiCad/KiCad.app/Contents/Frameworks/Python.framework/Versions/Current/bin/python3` |
| Linux | the system `python3`, with KiCad's packages installed |

```sh
"$KICAD_PYTHON" hardware/gen_board.py     # writes every variant
"$KICAD_PYTHON" hardware/make_outputs.py  # checks and exports each
```

Either takes a pitch as an argument to do just one: `gen_board.py 25.4`.

Needs [KiCad](https://www.kicad.org/) 9 or later — the author used 10.
...
and the renders. All three pass clean for both variants at the moment.
...
- a header breaking out the unused pins (`D23`, `D25`, `D26`, `D32`, `D34`,
  `D35`, …) for sensors. Not `D19`: the firmware hands it to the SPI
  peripheral as MISO in `SPI.begin(SCLK_PIN, MISO_PIN, MOSI_PIN)`, and
